...
/// <see cref="D2DRenderTargetBitmapImpl.CreateCompatiblePooled"/> so layers share the device
...
internal static class D2DGpuOffscreenFactory
{
    ...
    public static IDrawingContextLayerImpl Create(PixelSize pixelSize, Vector dpi)
    {
        ...
        // Hold the lock across SetDpi + CreateCompatible so concurrent different-DPI
        // requests cannot race the shared seed's DPI.
        lock (s_lock)
        {
            var seed = EnsureSeedUnlocked(dpi);
            var dipSize = pixelSize.ToSizeWithDpi(dpi);
            return D2DRenderTargetBitmapImpl.CreateCompatiblePooled(seed, dipSize);
        }
    }

    private static ID2D1DeviceContext EnsureSeedUnlocked(Vector dpi)
    {
