...
namespace Avalonia.Direct2D1.Media
{
    internal class FontManagerImpl : IFontManagerImpl
    {
        ...
        public string[] GetInstalledFontFamilyNames(bool checkForUpdates = false)
        {
            ...
            {
                fontFamilies[i] = Direct2D1FontCollectionCache.InstalledFontCollection.GetFontFamily((uint)i).FamilyNames.GetString(0);
            }
            ...
            FontWeight fontWeight, FontStretch fontStretch, CultureInfo culture, out Typeface typeface)
        {
            ...
            for (var i = 0; i < familyCount; i++)
            {
                var font = Direct2D1FontCollectionCache.InstalledFontCollection.GetFontFamily((uint)i)
        ...
        public bool TryCreateGlyphTypeface(Stream stream, FontSimulations fontSimulations, out IGlyphTypeface glyphTypeface)
        {
            ...
            var fontCollection = Direct2D1Platform.DirectWriteFactory.CreateCustomFontCollection(fontLoader, fontLoader.Key.PositionPointer, (uint)fontLoader.Key.RemainingLength);
